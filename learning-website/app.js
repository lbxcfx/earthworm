...
function startMode(mode) {
  ...
  switch (mode) {
    ...
    case 'match':
      ...
      break;
    case 'pairing':
      gameState.totalItems = 1; // 配对是一次完成
      break;
    case 'sentence':
    case 'fill':
...
function renderPairingMode(container) {
  const unit = getCurrentUnit();
  
  // 随机选择6-8个单词用于配对
  const wordCount = Math.min(8, unit.vocabulary.length);
  const selectedWords = shuffleArray([...unit.vocabulary]).slice(0, wordCount);
  
  pairingState.pairs = selectedWords.map(w => ({
    chinese: w.chinese,
    english: w.english
  }));
  
  pairingState.shuffledChinese = shuffleArray([...pairingState.pairs.map(p => p.chinese)]);
  pairingState.shuffledEnglish = shuffleArray([...pairingState.pairs.map(p => p.english)]);
  pairingState.selectedChinese = null;
  pairingState.selectedEnglish = null;
  pairingState.matchedPairs = [];
  pairingState.isProcessing = false;
  
  container.innerHTML = `
    <div class="pairing-container">
      <div class="pairing-instruction">
        <h3>🔗 点击配对中英文单词</h3>
        <p>可以先选中文或先选英文，匹配正确会变绿，然后变灰表示完成</p>
      </div>
      <div class="pairing-columns">
        <div class="pairing-column chinese-column">
          <div class="column-header">🇨🇳 中文</div>
          ${pairingState.shuffledChinese.map((word, i) => `
            <div class="pairing-item chinese-item" id="chinese-${i}" onclick="selectPairingItem('chinese', ${i}, '${word}')">${word}</div>
          `).join('')}
        </div>
        <div class="pairing-column english-column">
          <div class="column-header">🇬🇧 英文</div>
          ${pairingState.shuffledEnglish.map((word, i) => `
            <div class="pairing-item english-item" id="english-${i}" onclick="selectPairingItem('english', ${i}, '${word}')">${word}</div>
          `).join('')}
        </div>
      </div>
      <div class="pairing-progress">
        <span id="pairingProgressText">已配对: 0 / ${pairingState.pairs.length}</span>
      </div>
    </div>
  `;
}

function selectPairingItem(type, index, word) {
  if (pairingState.isProcessing) return;
  ...
  // 取消之前的选择
  if (type === 'chinese') {
    if (pairingState.selectedChinese !== null) {
      const prevEl = document.getElementById(`chinese-${pairingState.selectedChinese.index}`);
      if (prevEl && !prevEl.classList.contains('matched')) {
        prevEl.classList.remove('selected');
      }
    }
    pairingState.selectedChinese = { index, word };
    element.classList.add('selected');
    
    // 播放对应的英文发音
    const pair = pairingState.pairs.find(p => p.chinese === word);
    if (pair) {
      playWordSound(pair.english);
    }
  } else {
    if (pairingState.selectedEnglish !== null) {
      const prevEl = document.getElementById(`english-${pairingState.selectedEnglish.index}`);
      if (prevEl && !prevEl.classList.contains('matched')) {
        prevEl.classList.remove('selected');
      }
    }
    pairingState.selectedEnglish = { index, word };
    element.classList.add('selected');
  ...
  // 检查是否两边都选了
  if (pairingState.selectedChinese && pairingState.selectedEnglish) {
    checkPairingMatch();
  }
}

function checkPairingMatch() {
  pairingState.isProcessing = true;
  
  const chineseWord = pairingState.selectedChinese.word;
  const englishWord = pairingState.selectedEnglish.word;
  const chineseEl = document.getElementById(`chinese-${pairingState.selectedChinese.index}`);
  const englishEl = document.getElementById(`english-${pairingState.selectedEnglish.index}`);
  
  // 检查是否匹配
  const isMatch = pairingState.pairs.some(p => p.chinese === chineseWord && p.english === englishWord);
  ...
  if (isMatch) {
    ...
    setTimeout(() => {
      ...
      pairingState.matchedPairs.push({ chinese: chineseWord, english: englishWord });
      ...
      // 检查是否全部完成
      if (pairingState.matchedPairs.length === pairingState.pairs.length) {
...
function updatePairingProgress() {
  ...
  if (progressText) {
    progressText.textContent = `已配对: ${pairingState.matchedPairs.length} / ${pairingState.pairs.length}`;
  }
  
  // 更新顶部进度条
  const percent = (pairingState.matchedPairs.length / pairingState.pairs.length) * 100;
  document.getElementById('progressFill').style.width = `${percent}%`;
  document.getElementById('progressText').textContent = `${pairingState.matchedPairs.length}/${pairingState.pairs.length}`;
}

function handlePairingComplete() {
  setTimeout(() => {
    addCoins(30);
    showFeedback(true, '配对完成！', `太棒了！你成功配对了所有${pairingState.pairs.length}组单词！`, 30);
    
    // 记录学习数据
    pairingState.pairs.forEach(pair => {
      recordLearnedWord(pair.english);
    });
